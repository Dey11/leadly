...
import { checkInMemoryRateLimit, createRateLimitResponse, AI_RATE_LIMIT_CONFIG } from "../lib/rate-limit";
...
function isValidDescription(description: string): { valid: boolean; error?: string } {
    const sanitized = sanitizeInput(description);

    if (sanitized.length < 30) {
        return { valid: false, error: "Please provide a more detailed description (at least 30 characters)." };
    }

    if (sanitized.length > 2000) {
        return { valid: false, error: "Description is too long. Please keep it under 2000 characters." };
    }

    const wordCount = sanitized.split(/\s+/).filter(Boolean).length;
    if (wordCount < 5) {
        return { valid: false, error: "Please provide at least 5 words describing your ideal customer." };
    }
    ...
    for (const pattern of suspiciousPatterns) {
        if (pattern.test(sanitized)) {
            return { valid: false, error: "Please provide a genuine description of your ideal customer." };
        }
    }
...
const icpFieldsSchema = z.object({
    name: z.string().max(100).describe("A short, memorable name for this ICP (e.g., 'E-commerce SMB Owners')"),
    summary: z.string().max(2000).describe("2-3 sentence summary of the business offer and who it helps"),
    targetPersona: z.string().max(2000).describe("Job titles, company size, industry, and key characteristics"),
    pains: z.string().max(2000).describe("Specific problems, frustrations, and triggers that drive them to seek solutions"),
    valueProposition: z.string().max(2000).describe("How this product/service solves their problems and the outcomes they can expect"),
    qualifyingSignals: z.string().max(2000).describe("Keywords, phrases, and behaviors that indicate high buying intent"),
    disqualifyingSignals: z.string().max(2000).describe("Characteristics that indicate someone is NOT a fit"),
});
...
export async function suggestIcp(req: Request, res: Response) {
    try {
        ...
        const rateLimit = checkInMemoryRateLimit(`ai:icp:${userId}`, AI_RATE_LIMIT_CONFIG);
        if (!rateLimit.allowed) {
            return res.status(429).json(createRateLimitResponse(rateLimit.retryAfterSeconds));
        }

        const parseResult = requestSchema.safeParse(req.body);
        if (!parseResult.success) {
            return res.status(400).json({ error: "Please provide a detailed description (30-2000 characters)." });
        }
